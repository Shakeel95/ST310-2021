---
title: "Week 7 seminar: lasso inference experiments"
author:
...
Before we start you will likely have to install the `selectiveInference` package. 
...
STEP 1: compute lasso path while storing information required for conditional (truncated) inference 

```{r}
lasso_path <- lar(X, Y)
```

STEP 2: Use AIC/BIC to choose model size and compute adjusted p-values

(in higher dimensions change mult to log(n) for BIC, log(p) for RIC
